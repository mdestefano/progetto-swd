...
![](images/example_transaction.png){width="900"}
...
4.  **Confidence (c):** For a rule A=\>B Confidence shows the percentage in which B is with A.![](images/confidence.png)
...
Let's understand this by an example. In the following example, you will
see why APRIORI is an effective algorithm and also generate strong
...
-   **Density** tells the percentage of non-zero cells in a sparse matrix. You can say it as the total number of smells that are detected divided by a possible number of smells in that matrix. You can
    calculate how many items were purchased by using density: *1269x223x0.006979119=1974.96.*
...
In `itemFrequencyPlot(tr,topN=20,type="absolute")` first argument is the transaction object to be plotted that is `tr`. `topN` allows you to plot top N highest frequency items. `type` can be `type="absolute"` or `type="relative"`. If absolute it will plot numeric frequencies of each item
independently. If relative it will plot how many times these items have appeared as compared to others.
...
-   **Total number of rules:** The set of *119* rules
...
Since there are 119 rules, let's print only top 10:
...
-   100% of the classes which have design smell 'Broken Modularization' with test smells: ar1, gf1 then they have the same test smells and the design smell 'Unutilized Abstraction'.

-   100% of the classes which have design smell 'Cyclic Hierarchy', will also have design smell 'Cyclically-dependent Modularization'.

#### Finding Rules related to given items

If you want to find out what causes influence on the smell of item X we can use `appearance` option in the `apriori` command. `appearance` gives us options to set **LHS (IF part)** and **RHS (THEN part)** of the rule.

For example, to find what smells can exists before the design smell 'Unutilized Abstraction' with test smells ar1 and gf1 appears in one class, we run the following line of code:

```{r}
unutilizedAbstraction.association.rules <- apriori(tr, parameter = list(supp=0.001, conf=0.8),appearance = list(default="lhs",rhs=("Unutilized Abstraction ar1 gf1")))


```

```{r}
inspect(head(unutilizedAbstraction.association.rules))
```

Similarly, to find the answer to the question *Class which has 'Multifaceted Abstraction' as Design Smell, and ar1, gf1 as test smells also has....* we will keep "Multifaceted Abstraction ar1 gf1"on *lhs*:

```{r}
multifacetedAbstraction.association.rules <- apriori(tr, parameter = list(supp=0.001, conf=0.8),appearance = list(lhs="Multifaceted Abstraction ar1 gf1",default="rhs"))
```

```{r}
inspect(head(multifacetedAbstraction.association.rules))
```
...
Graph-based techniques visualize association rules using vertices and edges where vertices are labeled with item names, and item sets or rules are represented as a second set of vertices. Items are connected
with item-sets/rules using directed arrows. Arrows pointing from items to rule vertices indicate LHS items and an arrow from a rule to an item indicates the RHS. The size and color of vertices often represent interest measures.
...
It shows that when the class has 'Multifaceted Abstraction ar1 et1 it1 gf1', with high probability It will also have 'Insuffficient Modularization ar1 et1 se1 mg1'.
