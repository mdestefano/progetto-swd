...
for dir in list_dir_designite:
    with open("..\\datasets\\designite\\" + dir + "\\DesignSmells.csv") as path:
        des_smells_df = pd.read_csv(path)

        if des_smells_df.shape[0] > 0:
            packages = pd.unique(des_smells_df[' Package Name'])
            smells = pd.unique(des_smells_df[' Design Smell'])
            ...
            for package in packages:
                ...
                for smell1 in smells:
                    ...
                    riga1 = des_smells_df.loc[
                        (des_smells_df[" Design Smell"] == smell1) & (
                                des_smells_df[" Package Name"] == package)]

                    if len(riga1) > 0:
                        for smell2 in smells:
                            riga2 = des_smells_df.loc[
                                (des_smells_df[" Design Smell"] == smell2) & (
                                        des_smells_df[" Package Name"] == package)]
            ...
            cooccurrence_matrix_percentage.to_csv("risultatiDes\\"+dir+".csv")
